...
(function(){
  ...
  // Tipos de falta grave por contexto:
  //  · Pool  → faltas operativas cometidas cuando el colaborador YA está en el hotel.
  //  · Entrevistas → faltas detectadas durante el proceso de selección.
  // En ambos casos la UI añade "Otro (especificar)".
  const GRAVE_TYPES_POOL = [
    { key:'robo', lbl:'Robo o sustracción de bienes', ic:'gpp_bad',
      desc:'<strong>Robo o sustracción de bienes</strong> del hotel, de huéspedes o de compañeros durante su jornada laboral.' },
    { key:'agresion', lbl:'Agresión física o verbal', ic:'sentiment_very_dissatisfied',
      desc:'<strong>Agredió física o verbalmente</strong> a un huésped, compañero o supervisor dentro del hotel.' },
    { key:'seguridad', lbl:'Falta grave a la seguridad o protocolos', ic:'report',
      desc:'<strong>Incumplió protocolos de seguridad</strong> del hotel, poniendo en riesgo a huéspedes, compañeros o las instalaciones.' },
    { key:'sustancias', lbl:'Consumo de alcohol o sustancias en turno', ic:'no_drinks',
      desc:'Se presentó o trabajó <strong>bajo los efectos de alcohol o sustancias</strong> durante su turno en el hotel.' },
    { key:'acoso', lbl:'Acoso o conducta indebida', ic:'person_off',
      desc:'Incurrió en <strong>acoso o conducta indebida</strong> hacia huéspedes o compañeros dentro del hotel.' },
  ];
  const GRAVE_TYPES_ENTREV = [
    { key:'acoso_entrev', lbl:'Acoso o conducta indebida', ic:'person_off',
      desc:'Mostró <strong>acoso o conducta indebida</strong> hacia el equipo de reclutamiento durante el proceso de selección.' },
    { key:'no_apto', lbl:'No apto para trabajar (estado de drogas o alcohol)', ic:'block',
      desc:'Se presentó <strong>en estado de drogas o alcoholización</strong>, quedando no apto para trabajar.' },
  ];
  const GRAVE_TYPES = [...GRAVE_TYPES_POOL, ...GRAVE_TYPES_ENTREV];
  ...
  const PEOPLE = [
    ...
    {id:'BL-0417', nm:'Patricia Núñez', doc:'CURP NUPP9102', tel:'+52 322 778 1245', pos:'Recepción', zona:'Costera', m:'grave',   fecha:'06 may 2026', entrev:true, by:'Carlos Méndez', byrl:'Manager de Reclutamiento'},
    ...
    {id:'BL-0411', nm:'Mariana Ortiz', doc:'CURP ORMM9248', tel:'+52 322 190 6634', pos:'Recepción', zona:'Poniente', m:'grave', fecha:'18 abr 2026', entrev:true, by:'QA · Operaciones', byrl:'Control de calidad'},
    ...
    {id:'BL-0408', nm:'Omar Vázquez', doc:'CURP VAOO8455', tel:'+52 322 228 8841', pos:'Botones', zona:'Centro', m:'grave', fecha:'07 abr 2026', entrev:false, by:'María González', byrl:'Reclutadora · Zona Centro'},
    ...
    {id:'BL-0406', nm:'Andrés Maldonado', doc:'CURP MAAA8233', tel:'+52 322 740 1198', pos:'Recepción', zona:'Centro', m:'grave', fecha:'30 mar 2026', entrev:false, by:'María González', byrl:'Reclutadora · Zona Centro'},
  ...
  PEOPLE.forEach(p=>{
    if(p.m==='faltas'){
      ...
      // Conserva quién aparece en los datos del registro ANTES de sobrescribir con
      // el reportante del hotel: si fue una Reclutadora, ella ejecutó el veto.
      const regByName = p.by, regByRole = p.byrl;
      p.recVeto = /reclutad/i.test(regByRole || '');   // veto ejecutado por la reclutadora
      p.hotel = h0;
      p.reporter = rep;
      p.inspector = INSPECTORES[p.zona] || 'Inspector de zona';
      p.by = rep.name;
      p.byrl = rep.role + ' · ' + h0;
      // ¿La falta grave se detectó en la ENTREVISTA (selección) o en la OPERACIÓN
      // (el colaborador YA trabajaba en el hotel)? Cambia por completo el contexto.
      // Respeta el flag explícito del registro; si no, se decide de forma determinista.
      p.entrev = (typeof p.entrev === 'boolean') ? p.entrev : (hashStr(p.id + 'rec') % 2 === 0);
      // Tag "Reclutador" del listado: aparece cuando el veto lo ejecutó la reclutadora
      // (en entrevista, o tras el reporte del hotel en operación).
      p.manual = p.entrev || p.recVeto;
      if(p.entrev){
        // Detectada DURANTE la entrevista → la reclutadora la documenta y veta
        // directamente. No hay reporte de hotel ni investigación del Inspector.
        p.gt = GRAVE_TYPES_ENTREV[hashStr(p.id + p.nm) % GRAVE_TYPES_ENTREV.length];
        ...
        p.nota = `Falta grave (${p.gt.lbl.toLowerCase()}) detectada durante el proceso de entrevista. Como reclutadora documenté la conducta y veté al candidato; no continúa en selección y queda en Blacklist.`;
        p.pruebas = (hashStr(p.id)%2) ? ['Acta_de_entrevista.pdf'] : [];
      } else if(p.recVeto){
        // Detectada en la OPERACIÓN, pero la RECLUTADORA ejecuta el veto: el hotel
        // reporta y la reclutadora revisa el caso, lo valida y lo coloca en Blacklist.
        p.gt = GRAVE_TYPES_POOL[hashStr(p.id + p.nm) % GRAVE_TYPES_POOL.length];
        p.vetadaPor = regByName; p.vetadaPorRol = regByRole;
        p.regCat = 'reclutadora'; p.regName = regByName; p.regRole = regByRole;
        p.nota = `Falta grave (${p.gt.lbl.toLowerCase()}) confirmada en ${h0}. El hotel documentó y reportó el incidente; la reclutadora revisó el caso, lo validó y colocó al colaborador en la Blacklist.`;
        p.pruebas = ['Reporte_incidente.pdf','Evidencia_foto.jpg','Acta_de_hechos.pdf'].slice(0, 2 + (hashStr(p.id)%2));
      } else {
        // Detectada en la OPERACIÓN → el hotel reporta y el Inspector de zona
        // investiga, valida, resuelve y veta.
        p.gt = GRAVE_TYPES_POOL[hashStr(p.id + p.nm) % GRAVE_TYPES_POOL.length];
        p.regCat  = 'inspector';
        p.regName = p.inspector;
        p.regRole = 'Inspector de zona · Zona ' + p.zona;
        p.nota = `Falta grave (${p.gt.lbl.toLowerCase()}) confirmada en ${h0}. El hotel documentó y reportó el incidente y el Inspector de zona lo investigó, validó y lo colocó en la Blacklist`;
        p.pruebas = ['Reporte_incidente.pdf','Evidencia_foto.jpg','Acta_de_hechos.pdf'].slice(0, 2 + (hashStr(p.id)%2));
      }
    }
  ...
  const state = { q:'', motivo:'all', zona:'', vetby:'all', periodo:'all', dateFrom:'', dateTo:'', calMonth:null, order:'recientes', view:'board', tablePage:1, selected:null, openDD:null, drawerTab:'datos',
  ...
  function render(){
    ...
    const VETBY_OPTS = [['all','Todos','groups'],['reclutadora','Reclutadora','badge'],['inspector','Inspector de zona','verified_user']];
    const VETBY_LBL = {all:'Todos',reclutadora:'Reclutadora',inspector:'Inspector de zona'};
    ...
    const TBL_PER = 12;
    const tblPages = Math.max(1, Math.ceil(list.length / TBL_PER));
    if(state.tablePage > tblPages) state.tablePage = tblPages;
    if(state.tablePage < 1) state.tablePage = 1;
    const tblSlice = list.slice((state.tablePage-1)*TBL_PER, state.tablePage*TBL_PER);
    const rows = tblSlice.length ? tblSlice.map(p=>{
      const m = MOTIVOS[p.m];
      // Vía de ingreso al blacklist — coherente con el detalle (drawer):
      const via = p.auto
        ? { ic:'bolt', lbl:'Automático', sub:'Sistema · 3 faltas', cls:'auto' }
        : p.entrev
          ? { ic:'record_voice_over', lbl:'En entrevista', sub:'Reclutadora', cls:'entrev' }
          : p.recVeto
            ? { ic:'flag', lbl:'En operación', sub:'Hotel → Reclutadora', cls:'oper' }
            : { ic:'verified_user', lbl:'En operación', sub:'Hotel → Inspector', cls:'oper' };
      // Quién procesó / decidió el veto (no el reportante del hotel)
      const byNm = p.regName || p.by;
      const byRl = p.regRole || p.byrl;
      return `<tr class="bl-row${state.selected===p.id?' row-selected':''}" onclick="window.__blOpen('${p.id}')">
        ...
        <td><span class="bl-tbl-via" data-v="${via.cls}"><span class="mi">${via.ic}</span><span class="via-txt"><span class="v-lbl">${via.lbl}</span><span class="v-sub">${via.sub}</span></span></span></td>
        ...
        <td>
          <div class="bl-tbl-by">
            <div class="av">${ini(byNm)}</div>
            <div class="by-txt"><div class="nm">${byNm}</div><div class="rl">${byRl}</div></div>
          </div>
        </td>
        <td class="bl-row-chev"><span class="mi">chevron_right</span></td>
      </tr>`;
    }).join('') : `<tr><td colspan="7"><div class="bl-empty"><span class="mi">block</span><div style="margin-top:8px;font-weight:600">Sin colaboradores para los filtros aplicados</div></div></td></tr>`;
    ...
    const tableHTML = `
      <div class="req-table-wrap">
        ...
        ${window.__omPager ? window.__omPager(list.length, state.tablePage, TBL_PER, 'window.__blTablePage') : ''}
      </div>`;
  ...
  function renderBlAsignacion(p){
    if(p.entrev){
      // Vetado en entrevista — nunca llegó a tener una asignación en un hotel.
      return `
        <div class="dr-section">
          <div class="dr-section-title">Asignación a hotel</div>
          <div style="padding:26px 18px;text-align:center;color:var(--ink-3);background:#fff;border:1px solid var(--line);border-radius:12px">
            <span class="mi" style="font-size:38px;color:var(--ink-4)">person_search</span>
            <div style="margin-top:8px;font-size:13.5px;font-weight:700;color:rgb(83,59,23)">Sin asignación a hotel</div>
            <div style="margin-top:4px;font-size:12px;color:var(--ink-3);max-width:300px;margin-left:auto;margin-right:auto">El candidato fue vetado <strong>durante el proceso de entrevista</strong>; no llegó a ser colaborador ni a recibir una plaza en ningún hotel.</div>
          </div>
        </div>
        <div class="dr-section">
          ...
          <div style="padding:24px 18px;text-align:center;color:var(--ink-3);background:#fff;border:1px solid var(--line);border-radius:12px">
            ...
            <div style="margin-top:6px;font-size:13px;font-weight:600;color:rgb(83,59,23)">Sin historial laboral</div>
            <div style="margin-top:3px;font-size:11.5px;color:var(--ink-3)">No alcanzó a trabajar en la plataforma.</div>
          </div>
        </div>`;
    }
    const a = p.asg;
  ...
  // Perfil migratorio determinista por candidato. 4 variantes:
  //  0 → SSN + ITIN completos · 1 → solo SSN · 2 → solo ITIN · 3 → sin documentación fiscal
  function migProfileFor(h){
    const v = h % 4;
    return {
      ssn:  v===0 || v===1,
      itin: v===0 || v===2,
      ssnNum:  String(1000 + (h % 8999)),
      itinNum: '9' + String(10000000 + (h % 89999999))
    };
  }
  // Filas SSN/ITIN del tab Datos — misma lógica que el Pool de Reclutamiento.
  function migDatosRows(mig){
    mig = mig || { ssn:true, itin:true, ssnNum:'', itinNum:'' };
    const hasSsn = !!mig.ssn, hasItin = !!mig.itin;
    if(!hasSsn && !hasItin){
      return '<div class="entrev-detail-row is-migrante" data-c="ssn" style="grid-column:1/-1"><div class="ic"><span class="mi">flight_land</span></div><div class="txt"><div class="lbl">Estatus migratorio</div><div class="val">Sin documentación fisc.</div></div><button class="doc-info-btn" style="margin-left:auto;align-self:center" onclick="event.stopPropagation();window.__showDocRetentionInfo(\'ssn\')" title="Información sobre retención fiscal"><span class="mi">help</span></button></div>';
    }
    let r='';
    r += hasSsn
      ? '<div class="entrev-detail-row" data-c="ssn"><div class="ic"><span class="mi">badge</span></div><div class="txt"><div class="lbl">SSN</div><div class="val">'+mig.ssnNum+'</div></div><span class="stat-tag legal"><span class="mi">verified</span>Legal</span></div>'
      : '<div class="entrev-detail-row" data-c="ssn"><div class="ic"><span class="mi">badge</span></div><div class="txt"><div class="lbl">SSN</div><div class="val" style="color:var(--ink-3);font-style:italic">—</div></div><span class="stat-tag falta"><span class="mi">hourglass_top</span>Pendiente</span></div>';
    r += hasItin
      ? '<div class="entrev-detail-row" data-c="ssn"><div class="ic"><span class="mi">badge</span></div><div class="txt"><div class="lbl">ITIN</div><div class="val">'+mig.itinNum+'</div></div><span class="stat-tag legal"><span class="mi">verified</span>Legal</span></div>'
      : '<div class="entrev-detail-row" data-c="ssn"><div class="ic"><span class="mi">badge</span></div><div class="txt"><div class="lbl">ITIN</div><div class="val" style="color:var(--ink-3);font-style:italic">—</div></div><span class="stat-tag falta"><span class="mi">hourglass_top</span>Pendiente</span></div>';
    return r;
  }
  function renderBlDocsList(mig){
    mig = mig || { ssn:true, itin:true };
    const ITEMS = [
      {name:'CV / Currículum',          ic:'description',  sub:'Precargado automáticamente desde el alta del reclutador · PDF · 420 KB', have:true},
      {name:'SSN',                      ic:'badge',        sub:'Social Security Number · 180 KB',           migration:true, mkey:'ssn',  have:!!mig.ssn},
      {name:'ITIN',                     ic:'description',  sub:'Individual Taxpayer Identification · 180 KB', migration:true, mkey:'itin', optional:true, have:!!mig.itin},
      {name:'Foto del colaborador',     ic:'photo_camera', sub:'Imagen reciente · JPG/PNG · 1.2 MB', have:true},
      {name:'Comprobante de domicilio', ic:'home',         sub:'Recibo de servicios reciente (≤3 meses) · PDF/JPG · 540 KB', have:true},
      {name:'Comprobante o diploma',    ic:'school',       sub:'Documento opcional subido por el colaborador · 860 KB', optional:true, have:true},
    ];
    const recibidos = ITEMS.filter(it=>it.have).length;
    return `
      <div class="dr-section">
        <div class="entrev-section-h" style="padding-bottom:8px">
          <h4 style="margin:0">Documentos</h4>
          <span class="pill"><span class="mi">attach_file</span>${recibidos}/${ITEMS.length} recibidos</span>
        </div>
        <div class="entrev-docs-list">
          ${ITEMS.map(it=>{
            const have = it.have;
            const ds = have ? 'done' : 'pending';
            const stLbl = have ? 'Recibido' : 'Pendiente';
            const stIc  = have ? 'check_circle' : 'hourglass_top';
            const subTxt = have ? it.sub : (it.migration ? 'Sin documento adjuntado' : 'Pendiente');
            const info = it.migration ? `<button class="doc-info-btn" onclick="event.stopPropagation();window.__showDocRetentionInfo('${it.mkey}')" title="Información sobre retención fiscal"><span class="mi">help</span></button>` : '';
            return `
            <div class="entrev-doc-row ${ds}">
              <div class="ic"><span class="mi">${it.ic}</span></div>
              <div class="txt">
                <div class="nm">${it.name}${it.optional?' <span class="entrev-doc-opt">OPCIONAL</span>':''}${info}</div>
                <div class="sub">${subTxt}</div>
              </div>
              <span class="stat"><span class="mi">${stIc}</span>${stLbl}</span>
              ${have?`<button class="act" title="Ver" onclick="window.toast&&window.toast('Vista previa de ${it.name}','visibility')"><span class="mi">visibility</span></button>
  ...
  function renderBlActividad(p){
    const items = [];
    if(p.entrev){
      // Vetado DURANTE la entrevista — nunca llegó a ser colaborador del hotel.
      items.push({ when:'En proceso de entrevista', what:`<strong>Responsable:</strong> ${p.vetadaPor} (${p.vetadaPorRol}) · <strong>Comentario:</strong> "Candidato en proceso de selección para ${p.pos} · Zona ${p.zona}."` });
      items.push({ when:'Falta grave detectada en entrevista', what:`<strong>Fecha:</strong> ${p.fecha} · <strong>Responsable:</strong> ${p.vetadaPor} (${p.vetadaPorRol}) · <strong>Comentario:</strong> "${p.gt.lbl} durante el proceso de entrevista. La reclutadora documentó la conducta — sin investigación operativa."` });
      items.push({ when:'Ingreso a Blacklist', what:`<strong>Fecha:</strong> ${p.fecha} · <strong>Responsable:</strong> ${p.vetadaPor} (${p.vetadaPorRol}) · <strong>Comentario:</strong> "Veto permanente · estado Negro · sin apelación. El candidato no continúa en selección."` });
    } else {
      ...
      if(p.auto && p.faltas){
        ...
        if(p.recVeto){
          items.push({ when:'Revisión y veto · Reclutadora', what:`<strong>Responsable:</strong> ${p.regName} (${p.regRole}) · <strong>Comentario:</strong> "La reclutadora revisó el reporte del hotel, lo validó y ejecutó el veto."` });
        } else {
  ...
  function renderDrawer(){
    ...
    const datosHTML = `
      <div class="dr-section">
        <div class="dr-section-title">Datos del candidato</div>
    ...
    const laboralHTML = `
      <div class="dr-section">
        ...
        <div class="entrev-detail-grid" style="margin:0">
          ...
          ${p.entrev ? '' : `<div class="entrev-detail-row" data-c="mod"><div class="ic"><span class="mi">schedule</span></div><div class="txt"><div class="lbl">Modalidad</div><div class="val">${p.mod}</div></div></div>`}
      ...
      ${p.entrev ? `
      <div class="dr-section">
        <div class="dr-section-title">Disponibilidad horaria</div>
        <div style="padding:24px 18px;text-align:center;color:var(--ink-3);background:#fff;border:1px solid var(--line);border-radius:12px">
          <span class="mi" style="font-size:36px;color:var(--ink-4)">event_busy</span>
          <div style="margin-top:6px;font-size:13px;font-weight:600;color:rgb(83,59,23)">Sin schedule definido</div>
          <div style="margin-top:3px;font-size:11.5px;color:var(--ink-3);max-width:300px;margin-left:auto;margin-right:auto">La modalidad y el horario de trabajo no se capturan en la entrevista; el candidato fue vetado antes de definir su disponibilidad.</div>
        </div>
      </div>` : renderBlSched(p)}`;
    ...
    // ---------- TAB: Documentos ----------
    const docsHTML = renderBlDocsList(p.mig);
    ...
    const blacklistHTML = `
      <div class="dr-section">
        <div class="dr-section-title">Motivo del veto</div>
      ...
      <div class="dr-section">
        <div class="dr-section-title">Ingreso a blacklist</div>
        <div class="entrev-detail-grid" style="margin:0 0 16px">
          ${p.auto
            ...
            : p.entrev
              ? `<div class="entrev-detail-row"><div class="ic"><span class="mi">record_voice_over</span></div><div class="txt"><div class="lbl">Detectado en</div><div class="val">Proceso de entrevista</div></div></div>
        ...
        <div class="entrev-section-h" style="padding-bottom:8px">
          <h4 style="margin:0">${p.auto?'Registro':p.entrev?'Responsable':'Responsables'}</h4>
        </div>
        <div class="entrev-emerg-card">
          ${p.auto
            ...
            : p.entrev
              ? `<div class="entrev-emerg-row"><div class="ic"><span class="mi">how_to_reg</span></div><div class="txt"><div class="lbl">Vetado en entrevista por</div><div class="val">${p.vetadaPor}<span class="sub">${p.vetadaPorRol} · detectó la falta y registró el veto</span></div></div></div>`
              : `<div class="entrev-emerg-row"><div class="ic"><span class="mi">flag</span></div><div class="txt"><div class="lbl">Reportado por</div><div class="val">${p.reporter.name}<span class="sub">${p.reporter.role} · ${p.hotel}</span></div></div></div>
               ${p.recVeto
                 ? `<div class="entrev-emerg-row"><div class="ic"><span class="mi">how_to_reg</span></div><div class="txt"><div class="lbl">Revisado y vetado por</div><div class="val">${p.regName}<span class="sub">${p.regRole} · validó el reporte y ejecutó el veto</span></div></div></div>`
    ...
    // Vetado en entrevista (Patricia, Mariana): no pasó la selección, así que no
    // hay datos laborales ni historial de asignaciones → se ocultan ambas tabs.
    const safeTab = (p.entrev && (tab==='laboral'||tab==='historial')) ? 'datos' : tab;
    const bodyHTML = safeTab==='laboral' ? laboralHTML : safeTab==='historial' ? histHTML : safeTab==='docs' ? docsHTML : safeTab==='blacklist' ? blacklistHTML : datosHTML;
    const tabs = p.entrev
      ? [['datos','Datos','badge'],['docs','Documentos','description'],['blacklist','Blacklist','block']]
      : [['datos','Datos','badge'],['laboral','Laboral','work_outline'],['historial','Historial','history'],['docs','Documentos','description'],['blacklist','Blacklist','block']];
    ...
    drw.innerHTML = `
      ...
      <div class="recl-drawer-tabs-wrap">
        <div class="recl-drawer-tabs">
          ${tabs.map(t=>`<div class="recl-drawer-tab ${safeTab===t[0]?'active':''}" onclick="window.__blSetTab('${t[0]}')"><span class="mi">${t[2]}</span>${t[1]}</div>`).join('')}
        </div>
  ...
  function renderPoolProfile(){
    ...
    // Modo "candidato" (Ver perfil desde Requisiciones): respeta los datos reales
    // de la card (posición, zona, modalidad, inglés e ID C-####) en la cabecera.
    if(src && src._over) Object.assign(pp, src._over);
    const sm = SEMAFORO[pp.st] || { lbl:'—', color:'#9A8C7E' };
    ...
    // Datos, Laboral e Historial — reutilizan EXACTAMENTE el mismo componente que el
    // Pool de colaboradores en Reclutamiento, POR ESTADO del semáforo (incluye Estado
    // en blacklist + Historial en Datos; leyenda del schedule + Semáforo actual en
    // Laboral; acordeón de asignaciones en Historial). En modo "candidato" (Ver perfil
    // desde Requisiciones) inyectamos la migración REAL del candidato (pp.mig) para que
    // el SSN/ITIN del tab Datos coincida con su tab Documentos.
    if(state.poolCandMode && pp.mig) window.__reclCandMigOverride = pp.mig;
    ...
    window.__reclCandMigOverride = null;
    const docsHTML = renderBlDocsList(pp.mig);
    ...
    drw.innerHTML = `
      ...
      ${state.poolCandMode
        ...
        <button class="bl-prof-foot-btn" onclick="window.__requiAssignFromPerfil && window.__requiAssignFromPerfil('${(state.poolCandId||'')}')"><span class="mi">person_add</span>Asignar colaborador</button>
      </div>`
        : `<div class="bl-prof-foot">
        <button class="bl-prof-foot-btn bl-prof-foot-btn--veto" onclick="window.__blPoolProfilePick()">Seleccionar para vetar</button>
      </div>`}
    `;
    drw.classList.add('open');
  ...
  /* ---------------- ADD MODAL ---------------- */
  // Estados del proceso de entrevista (mismas etiquetas/colores que el módulo
  // de Reclutamiento › Entrevistas). El reclutador puede vetar a un candidato
  // en CUALQUIER estado del proceso, no solo a los del pool.
  const E_STATES = {
    pendApp:    { lbl:'Pendiente de App',     color:'#FF7A00', ic:'hourglass_top' },
    pendVal:    { lbl:'Pendiente de Validar', color:'#C79400', ic:'verified' },
    inconcluso: { lbl:'Inconcluso',           color:'#8B5A2B', ic:'remove_circle' },
    aband:      { lbl:'Abandonado',           color:'#9AA0A6', ic:'person_off' },
    valid:      { lbl:'Validado',             color:'#1FA84A', ic:'how_to_reg' },
  };
  const E_ORDER = ['pendApp','pendVal','inconcluso','aband','valid'];
  // Candidatos en proceso de entrevista — se leen EN VIVO del módulo de
  // Reclutamiento (window.__ENTREV_LIST) para que "Ver perfil" abra exactamente
  // la misma ficha. Se mapea el estado real (st/rejKind) al estado de display.
  function eDispState(c){
    if(c.st==='valid') return 'valid';
    if(c.st==='pendApp') return 'pendApp';
    if(c.st==='pendVal') return 'pendVal';
    if(c.st==='rej'){ if(c.rejKind==='aband') return 'aband'; if(c.rejKind==='bl') return 'aband'; return 'inconcluso'; }
    return 'pendVal';
  }
  function getEntrevAdd(){
    const src = (window.__ENTREV_LIST || []);
    return src.map(c=>({
      id:c.id, nm:c.name, doc:c.doc, role:c.pos, zona:c.zone,
      mod:(c.mod||'').replace(/\n/g,' ').trim(), origen:(c.origen||'').trim(), st:eDispState(c)
    }));
  }
  function renderAddModal(){
    ...
    // ===== TAB ENTREVISTAS — candidatos en proceso, en cualquier estado =====
    a.eflt = a.eflt || {pos:'',zona:'',mod:'',estado:''};
    const ENTREV_ADD = getEntrevAdd();
    const eq = (a.esearch||'').trim().toLowerCase();
    const euniq = (arr)=>[...new Set(arr)];
    const E_FILTERS = [
      ['pos',    'Posición', 'Todas',  euniq(ENTREV_ADD.map(x=>x.role)).map(v=>[v,v])],
      ['zona',   'Zona',     'Todas',  euniq(ENTREV_ADD.map(x=>x.zona)).map(v=>[v,v])],
      ['mod',    'Modalidad','Todas',  euniq(ENTREV_ADD.map(x=>x.mod).filter(Boolean)).map(v=>[v,v])],
      ['estado', 'Estado',   'Todos',  E_ORDER.filter(k=>ENTREV_ADD.some(x=>x.st===k)).map(k=>[k, E_STATES[k].lbl])],
    ];
    const eFkey = { pos:'role', zona:'zona', mod:'mod', estado:'st' };
    const eFiltered = ENTREV_ADD.filter(x=>{
      if(eq && !(x.nm.toLowerCase().includes(eq) || (x.doc||'').toLowerCase().includes(eq) || x.zona.toLowerCase().includes(eq) || x.role.toLowerCase().includes(eq) || x.id.toLowerCase().includes(eq))) return false;
      for(const k in eFkey){ if(a.eflt[k] && x[eFkey[k]] !== a.eflt[k]) return false; }
      return true;
    });
    const eFilterChip = ([key,label,allLbl,opts])=>{
      const cur = a.eflt[key];
      const curLbl = cur ? ((opts.find(o=>o[0]===cur)||[])[1] || allLbl) : allLbl;
      const open = a.openFilter==='e_'+key;
      const dot = (v)=> key==='estado' ? `<span class="recl-fdd-dot" style="background:${(E_STATES[v]||{}).color||'#9A8C7E'}"></span>` : '';
      return `<div class="recl-filter-grp${cur?' active':''}${open?' open':''}" onclick="event.stopPropagation();window.__blAddEToggleFilterDD('${key}')">
        <span class="lbl-grp">${label}</span>
        <span class="val">${cur?dot(cur):''}${curLbl}</span>
        <span class="mi">expand_more</span>
        <div class="recl-fdd" onclick="event.stopPropagation()">
          <div class="recl-fdd-item all" onclick="window.__blAddEPickFilter('${key}','')"><span>${allLbl}</span></div>
          ${opts.map(([v,l])=>`<div class="recl-fdd-item${cur===v?' selected':''}" onclick="window.__blAddEPickFilter('${key}','${v}')">${dot(v)}<span>${l}</span><span class="mi">check</span></div>`).join('')}
        </div>
      </div>`;
    };
    const eFiltersHTML = E_FILTERS.map(eFilterChip).join('');
    const eAnyFlt = Object.values(a.eflt).some(Boolean);
    const eClearHTML = eAnyFlt ? `<div class="bl-pool-clearf" onclick="window.__blAddEClearFilters()"><span class="mi">close</span>Limpiar</div>` : '';
    const eRowsHTML = eFiltered.length ? eFiltered.map(x=>{
      const sel = a.colab && a.colab.nm===x.nm;
      const est = E_STATES[x.st] || { lbl:'—', color:'#9A8C7E', ic:'help' };
      return `<div class="ram-card${sel?' selected':''}" onclick="window.__blAddEPickRow('${x.id}')">
        <div class="ram-card-h">
          <div class="ram-card-check"></div>
          <div class="ram-card-av" style="background:${gradAv(x.nm)}">${ini(x.nm)}</div>
          <div class="ram-card-main">
            <div class="ram-card-name">
              ${x.nm}
              <span class="ram-card-doc">${x.id}</span>
              <span class="ram-card-st"><span class="dot" style="background:${est.color}"></span>${est.lbl}</span>
            </div>
            <div class="ram-tags">
              <span class="ram-tag pos"><span class="mi">check_circle</span>${x.role}</span>
              <span class="ram-tag zone"><span class="mi">place</span>${x.zona}</span>
              ${x.st==='pendApp'
                ? (x.origen?`<span class="ram-tag origen"><span class="mi">${/refer/i.test(x.origen)?'group':/difus/i.test(x.origen)?'share':'campaign'}</span>${x.origen}</span>`:'')
                : (x.mod?`<span class="ram-tag mod">${x.mod}</span>`:'')}
            </div>
          </div>
          <div class="ram-card-right">
            <button class="ram-card-perfil" onclick="event.stopPropagation();window.__blAddEVerPerfil('${x.id}')"><span class="mi">visibility</span>Ver perfil</button>
          </div>
        </div>
      </div>`;
    }).join('') : `<div class="bl-pool-empty">Sin candidatos en entrevistas para los filtros aplicados</div>`;
    const entrevBlock = `
      <div class="bl-pool-search">
        <span class="mi">search</span>
        <input placeholder="Buscar en entrevistas por nombre, ID, DOC o zona…" value="${(a.esearch||'').replace(/"/g,'&quot;')}" oninput="window.__blAddESearch(this.value)">
      </div>
      <div class="bl-pool-filters">${eFiltersHTML}${eClearHTML}</div>
      <div class="bl-pool-meta"><strong>${eFiltered.length}</strong> ${eFiltered.length===1?'candidato':'candidatos'} en entrevistas${a.colab?` · seleccionado: <strong>${a.colab.nm}</strong>`:''}</div>
      <div class="bl-pool-list">${eRowsHTML}</div>`;

    // Tabs Entrevistas / Pool (mismo control que Reclutamiento)
    const tab = a.tab || 'pool';
    const tabsHTML = `
      <div class="recl-segmented requi-tabs-full" style="margin-bottom:14px">
        <button class="${tab==='entrev'?'active':''}" onclick="window.__blAddTab('entrev')">
          <span class="mi">how_to_reg</span>
          <span class="requi-tab-label">Entrevistas</span>
          <span class="requi-tab-count" data-tone="info">${ENTREV_ADD.length}</span>
        </button>
        <button class="${tab==='pool'?'active':''}" onclick="window.__blAddTab('pool')">
          <span class="mi">groups</span>
          <span class="requi-tab-label">Pool de colaboradores</span>
          <span class="requi-tab-count" data-tone="ok">${POOL.length}</span>
        </button>
      </div>`;
    const field1Block = (tab==='entrev' ? entrevBlock : colabBlock);
    ...
    const GT_LIST = (a.tab==='entrev') ? GRAVE_TYPES_ENTREV : GRAVE_TYPES_POOL;
    const gtLabel = gt==='otro' ? 'Otro (especificar)' : (gt ? ((GT_LIST.find(g=>g.key===gt)||{}).lbl||'') : '');
    const graveSelOpts = GT_LIST.map(g=>`<div class="recl-select-opt${gt===g.key?' selected':''}" onclick="window.__blAddPickGrave('${g.key}')">${g.lbl}<span class="mi">check</span></div>`).join('')
    ...
    const isEntrev = a.tab==='entrev';
    const graveOk = a.graveType && (a.graveType!=='otro' || (a.graveOtro||'').trim().length>=4);
    const valid = a.colab && graveOk && a.nota.trim().length>=8 && (isEntrev || a.pruebas.length>=1);
    ...
    m.innerHTML = `
      ...
      <div class="bl-modal-body" onclick="window.__blAddBodyClick(event)">
        ...
        <div class="bl-field" style="margin-bottom:4px">
          <div class="bl-field-lbl"><span class="num">4</span>Pruebas / evidencia ${isEntrev ? '<span class="bl-opt">(opcional)</span>' : '<span class="req">*</span>'}</div>
          <div class="bl-field-hint">${isEntrev ? 'Opcional en entrevistas. Si cuentas con evidencia (capturas, correos, reportes), adjúntala para reforzar el veto.' : 'Adjunta fotos, reportes o documentos que respalden el veto. Quedarán en el expediente del colaborador.'}</div>
  ...
  /* ---------------- ACTIONS ---------------- */
  window.__blSearch = (v)=>{ state.q=v; state.tablePage=1; render(); };
  window.__blMotivo = (k)=>{ state.motivo=k; state.openDD=null; state.tablePage=1; render(); };
  window.__blZona = (z)=>{ state.zona=z; state.openDD=null; state.tablePage=1; render(); };
  window.__blVetby = (v)=>{ state.vetby=v; state.openDD=null; state.tablePage=1; render(); };
  window.__blPeriodo = (p)=>{ state.periodo=p; state.tablePage=1; if(p!=='custom'){ state.openDD=null; } else { const base = state.dateFrom ? new Date(state.dateFrom+'T00:00:00') : new Date(NOW_BL); state.calMonth = new Date(base.getFullYear(), base.getMonth(), 1); } render(); };
  ...
  window.__blView = (v)=>{ state.view=v; state.openDD=null; state.tablePage=1; render(); };
  window.__blTablePage = (n)=>{ state.tablePage=n; render(); };
  window.__blDD = (k,ev)=>{ ev.stopPropagation(); state.openDD = state.openDD===k?null:k; render(); };
  window.__blClear = ()=>{ state.q=''; state.zona=''; state.motivo='all'; state.vetby='all'; state.periodo='all'; state.dateFrom=''; state.dateTo=''; state.tablePage=1; render(); };
  ...
  // add modal
  window.__blAddOpen = ()=>{ state.add={open:true,tab:'pool',colab:null,motivo:'grave',graveType:null,graveOtro:'',nota:'',search:'',esearch:'',flt:{pos:'',zona:'',mod:'',status:''},eflt:{pos:'',zona:'',mod:'',estado:''},langs:{},openFilter:null,pruebas:[]}; renderAddModal(); };
  ...
  // Tab Entrevistas / Pool dentro del modal de veto
  window.__blAddTab = (t)=>{ state.add.tab=t; state.add.openFilter=null; state.add.graveType=null; renderAddModal(); };
  window.__blAddESearch = (v)=>{ state.add.esearch=v; renderAddModal(); };
  window.__blAddEToggleFilterDD = (k)=>{ state.add.openFilter = state.add.openFilter===('e_'+k)?null:('e_'+k); renderAddModal(); };
  window.__blAddEPickFilter = (k,v)=>{ state.add.eflt=state.add.eflt||{pos:'',zona:'',mod:'',estado:''}; state.add.eflt[k]=v; state.add.openFilter=null; renderAddModal(); };
  window.__blAddEClearFilters = ()=>{ state.add.eflt={pos:'',zona:'',mod:'',estado:''}; state.add.openFilter=null; renderAddModal(); };
  window.__blAddEPickRow = (id)=>{
    const c = getEntrevAdd().find(x=>x.id===id); if(!c) return;
    const cur = state.add.colab;
    state.add.colab = (cur && cur.nm===c.nm) ? null : { nm:c.nm, doc:c.doc, pos:`${c.role} · Zona ${c.zona}`, _entrev:true, _eid:c.id };
    renderAddModal();
  };
  // "Ver perfil" — abre la MISMA ficha del módulo de Entrevistas en modo veto
  window.__blAddEVerPerfil = (id)=>{
    if(typeof window.__entrevOpenForVeto === 'function') window.__entrevOpenForVeto(id);
  };
  // CTA "Vetar colaborador" dentro de esa ficha → vuelve al modal de blacklist
  // (tab Entrevistas) con el candidato ya seleccionado.
  window.__blVetoFromEntrev = (id)=>{
    if(typeof window.__entrevCloseDrawer === 'function') window.__entrevCloseDrawer();
    const c = getEntrevAdd().find(x=>x.id===id);
    if(!state.add || !state.add.open) window.__blAddOpen();
    state.add.tab = 'entrev';
    if(c) state.add.colab = { nm:c.nm, doc:c.doc, pos:`${c.role} · Zona ${c.zona}`, _entrev:true, _eid:c.id };
    renderAddModal();
  };
  window.__blPoolProfile = (nm)=>{ state.poolCandMode=false; state.poolView = POOL.find(x=>x.nm===nm) || null; state.poolTab='datos'; renderPoolProfile(); };
  window.__blPoolProfileClose = ()=>{ state.poolView=null; state.poolCandMode=false; renderPoolProfile(); };
  // Abre el MISMO drawer de "Ver perfil" (tabs Datos/Laboral/Historial/Documentos,
  // estilo Pool de colaboradores) para un candidato de Requisiciones. La cabecera
  // usa los datos reales de la card; el cuerpo reutiliza el componente por estado
  // del semáforo (verdef = Disponible · amarillo = Disp. voluntario). Sin pie de
  // "Seleccionar para vetar" — aquí solo es consulta.
  window.__blPoolProfileForCand = (c)=>{
    if(!c) return;
    const st = /voluntar/i.test(c.dispo||'') ? 'amarillo' : 'verdef';
    const src = {
      nm:c.nm, doc: c.doc || String(c.id||'').replace(/\D/g,'') || '0000', st,
      pos: (c.pos||'Colaborador') + ' · Zona ' + (c.zone||'Centro'),
      _over: {
        cid: 'ID ' + (c.id||''),
        pos: c.pos || '', zona: c.zone || 'Centro',
        mod: c.mod || '',
        engLbl: c.eng ? ('Inglés '+c.eng) : undefined,
      }
    };
    state.poolView = src; state.poolCandMode = true; state.poolCandId = c.id || ''; state.poolTab='datos';
    renderPoolProfile();
  };
  ...
  window.__blAddGraveOtro = (v)=>{ state.add.graveOtro=v; const b=document.querySelector('#bl-add-modal .bl-modal-btn.confirm'); const a=state.add; const graveOk=a.graveType&&(a.graveType!=='otro'||(a.graveOtro||'').trim().length>=4); const valid=a.colab&&graveOk&&a.nota.trim().length>=8&&(a.tab==='entrev'||(a.pruebas||[]).length>=1); if(b) b.disabled=!valid; };
  window.__blAddNota = (v)=>{ state.add.nota=v; const b=document.querySelector('#bl-add-modal .bl-modal-btn.confirm'); const a=state.add; const graveOk=a.graveType&&(a.graveType!=='otro'||(a.graveOtro||'').trim().length>=4); const valid=a.colab&&graveOk&&v.trim().length>=8&&(a.tab==='entrev'||(a.pruebas||[]).length>=1); if(b) b.disabled=!valid; };
  window.__blAddConfirm = ()=>{
    const a = state.add;
    const graveOk = a.graveType && (a.graveType!=='otro' || (a.graveOtro||'').trim().length>=4);
    if(!(a.colab&&graveOk&&a.nota.trim().length>=8&&(a.tab==='entrev'||(a.pruebas||[]).length>=1))) return;
    ...
    PEOPLE.unshift({
      ...
      by:'María González', byrl:'Reclutadora · Zona Centro',
      hotel:'Hotel asignado', reporter:{name:'María González', role:'Reclutadora'}, inspector:'Inspector de zona'
    });
